...
class MSPDZ { public:
	...
	void Online_mul(uint64_t * x, uint64_t * y, uint64_t * mac_x, uint64_t * mac_y, uint64_t * output, uint64_t *output_mac) {
        ...
        for(int i = 0; i < sz; ++i){
			if(party==1){
				output[i] = c[i]+db[i]+r[i]+de[i]+f_t[i];
			}else{
				output[i] = c[i]+db[i]+r[i];
			}
        }
		for(int i=0;i<mat_sz;i++){
			output_mac[i] = mac_c[i]+mac_db[i]+mac_r[i]+mac_de[i]+mac_f_t[i];
		}
